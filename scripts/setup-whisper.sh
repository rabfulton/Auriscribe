...
# Disable by setting AURISCRIBE_VULKAN=0.
# By default, we require Vulkan builds (CPU-only Whisper is too slow for Auriscribe).
# Set AURISCRIBE_REQUIRE_VULKAN=0 to allow CPU-only builds.
#
# Back-compat: the old XFCE_WHISPER_* env vars are still honored.
GGML_VULKAN_FLAG=""
VULKAN_SETTING="${AURISCRIBE_VULKAN:-${XFCE_WHISPER_VULKAN:-1}}"
REQUIRE_VULKAN_SETTING="${AURISCRIBE_REQUIRE_VULKAN:-${XFCE_WHISPER_REQUIRE_VULKAN:-1}}"

if [ "${VULKAN_SETTING}" != "0" ]; then
    if command -v pkg-config >/dev/null 2>&1 && pkg-config --exists vulkan && command -v glslc >/dev/null 2>&1; then
        ...
        if [ "${REQUIRE_VULKAN_SETTING}" = "0" ]; then
            ...
            echo "Install Vulkan dev packages + glslc, or set AURISCRIBE_REQUIRE_VULKAN=0 to allow CPU-only."
            exit 1
        fi
    fi
else
    echo "AURISCRIBE_VULKAN=0 set; building whisper.cpp CPU-only."
fi
